...
public class BFSTraversal {
    ...
    public LinkedList<Node> getNodeListCopy() {
        LinkedList<Node> nodeListCopy = new LinkedList<>();
        nodeListCopy.addAll(nodeList);
        return nodeListCopy;
    }

    double getNepotismRatio() {
        return ((1.0 * getNumConnectedComps() / (1.0 * nodeList.size())));
    }

    public int getNumConnectedComps() {
        breadthFirstSearch(nodeList.getFirst(), getNodeListCopy());
        return numConnectedComps;
    }

    public int getConnectedCompSize(Node source) {
        int counter = 0;
        LinkedList<Node> nodesLeft = getNodeListCopy();
